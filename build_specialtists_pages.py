#!/usr/bin/env python3
"""
build_specialists_pages.py
==========================
Builds /specialists/{specialty}/index.html pages combining:
  - NHS GPs (from gps.json) — always shown as a separate "NHS GPs" section
  - Private consultants (from private_clinics.json) — shown per specialty

Output structure:
  specialists/index.html                    ← hub listing all specialties
  specialists/{specialty}/index.html        ← per-specialty page

Each specialty page shows:
  - Private providers for that specialty
  - NHS GPs in London (relevant for: private gp, general medicine)
  - Borough filter chips
  - CQC rating badges
  - Phone, website, address

Run from repo root:
  python3 build_specialists_pages.py
"""
...
SPECIALTY_META = {
    ...
    "cosmetic":          {"emoji": "✨", "label": "Cosmetic & Aesthetic Medicine",  "desc": "Private cosmetic surgery and aesthetic medicine clinics in London for Botox, fillers, rhinoplasty and body treatments."},
    ...
    "diagnostics":       {"emoji": "🔭", "label": "Diagnostics & Imaging",         "desc": "Private diagnostic imaging and health screening clinics in London offering MRI, CT scans, ultrasound and blood tests."},
    "physiotherapy":     {"emoji": "💪", "label": "Physiotherapy & Rehab",         "desc": "Private physiotherapists and rehabilitation clinics in London for injury recovery, sports rehab and musculoskeletal treatment."},
    "travel":            {"emoji": "✈️", "label": "Travel Health & Vaccines",       "desc": "Private travel health clinics in London for vaccinations, malaria prevention and travel medicine advice."},
    "allergy":           {"emoji": "🌿", "label": "Allergy & Immunology",          "desc": "Private allergy clinics and immunologists in London for food allergy, hay fever, asthma and immune conditions."},
    "pain management":   {"emoji": "💊", "label": "Pain Management",               "desc": "Private pain management clinics and specialists in London for chronic pain, back pain and pain relief treatments."},
    ...
    "private gp":        {"emoji": "🏠", "label": "Private GP & General Medicine", "desc": "Private GP clinics in London for same-day appointments, health checks, prescriptions and general medical care."},
    "general medicine":  {"emoji": "🩻", "label": "General Medicine",              "desc": "Private general medicine clinics in London offering a broad range of specialist consultations and treatments."},
}
...
def cqc_class(r):
    m = {'Outstanding':'O','Good':'G','Requires improvement':'RI','Inadequate':'I'}
    return m.get(r, '')
...
def render_private_card(p):
    ...
    cqc_cls = cqc_class(rating)
    badge = f'<span class="cqc-badge cqc-{cqc_cls}">{cqc_label(rating)}</span>' if rating else ''
    borough_tag = f'<span class="tag borough">{p["borough"]}</span>' if p.get("borough") else ''
    spec_tags = ''.join(f'<span class="tag spec">{s}</span>' for s in (p.get('specialties') or [])[:2])
    actions = []
    if p.get('phone'):
        actions.append(f'<span class="btn-phone">📞 {p["phone"]}</span>')
    if p.get('cqc_url'):
        actions.append(f'<a class="btn btn-cqc" href="{p["cqc_url"]}" target="_blank" rel="noopener">CQC</a>')
    if p.get('website'):
        actions.append(f'<a class="btn btn-web" href="{p["website"]}" target="_blank" rel="noopener">Website</a>')
    return f"""<div class="card" data-borough="{p.get('borough','')}" data-type="private">
...
def render_nhs_card(g):
    ...
    actions = []
    if g.get('ph'):
        actions.append(f'<span class="btn-phone">📞 {g["ph"]}</span>')
    if g.get('cu'):
        actions.append(f'<a class="btn btn-cqc" href="{g["cu"]}" target="_blank" rel="noopener">CQC</a>')
...
(function(){
  ...
  function applyFilters(){
    ...
    document.getElementById('countPriv').textContent = privVis + ' private';
    document.getElementById('countNhs').textContent  = nhsVis + ' NHS';
  }
...
def build_specialty_page(spec, meta, private_records, nhs_records):
    ...
    # All boroughs across both sets
    boroughs = sorted(set(
        r.get('borough','') for r in private_records + nhs_records
        if r.get('borough','')
    ))
...
<div class="section-title">
  🏥 NHS GP Practices in London
  <span class="count">{len(nhs_records)}</span>
</div>
...
<title>{label} Clinics in London — NHS & Private | London Healthcare Directory</title>
<meta name="description" content="{desc}">
<link rel="canonical" href="https://londongp.directory/specialists/{slugify(spec)}/">
{CSS}
</head>
<body>
{NAV}
<div class="page-header">
  ...
  <div class="stats">
    ...
    <div class="stat"><strong id="countNhs">{len(nhs_records)} NHS</strong><span>NHS practices</span></div>
...
<div class="content">
  ...
  <div class="filter-bar">
    ...
    <div class="type-switch">
      ...
      <button class="type-btn" data-type="nhs">NHS</button>
    </div>
  </div>

  <div class="section-title">
    💊 Private {label}
    <span class="count">{len(private_records)}</span>
  </div>
...
<footer>
  Data: CQC · NHS ODS · GP Patient Survey · London Healthcare Directory<br>
...
def build_hub_page(specialty_counts):
    ...
    for spec in sorted(specialty_counts.keys(),
                       key=lambda s: specialty_counts[s], reverse=True):
        meta   = SPECIALTY_META.get(spec, {})
        emoji  = meta.get('emoji','🩺')
        label  = meta.get('label', spec.title())
        n_priv, n_nhs = specialty_counts[spec]
        slug   = slugify(spec)
        cards += f"""<a class="spec-card" href="/specialists/{slug}/">
  <div class="spec-emoji">{emoji}</div>
  <h2>{label}</h2>
  <p class="spec-count">
    <strong>{n_priv}</strong> private · <strong>{n_nhs}</strong> NHS
  </p>
</a>"""

    total_priv = sum(v[0] for v in specialty_counts.values())
    total_nhs  = sum(v[1] for v in specialty_counts.values())
...
<div class="page-header">
  ...
  <div class="stats">
    ...
    <div class="stat"><strong>{total_nhs}</strong><span>NHS practices</span></div>
...
def main():
    # Load private clinics
    if not PRIVATE_JSON.exists():
        print(f"ERROR: {PRIVATE_JSON} not found. Run fetch_private_clinics.py first.")
        return
    ...
    # Group private by specialty
    priv_by_spec = defaultdict(list)
    for r in private_all:
        if not r.get('name') or not r.get('borough'):
            continue
        for spec in (r.get('specialties') or ['general medicine']):
            priv_by_spec[spec].append(r)

    # NHS GPs only appear on "private gp" and "general medicine" specialty pages
    nhs_specs = {'private gp', 'general medicine'}

    # Count for hub
    all_specs = set(priv_by_spec.keys()) | nhs_specs
    specialty_counts = {}
    for spec in all_specs:
        n_priv = len(priv_by_spec.get(spec, []))
        n_nhs  = len(nhs_all) if spec in nhs_specs else 0
        if n_priv + n_nhs > 0:
            specialty_counts[spec] = (n_priv, n_nhs)

    # Build hub
    OUT_DIR.mkdir(parents=True, exist_ok=True)
    hub_html = build_hub_page(specialty_counts)
    ...
    for spec, (n_priv, _) in sorted(specialty_counts.items()):
        ...
        nhs_recs  = nhs_all if spec in nhs_specs else []
        meta      = SPECIALTY_META.get(spec, {"emoji":"🩺","label":spec.title(),"desc":""})

        spec_dir = OUT_DIR / slugify(spec)
        spec_dir.mkdir(parents=True, exist_ok=True)
        page = build_specialty_page(spec, meta, priv_recs, nhs_recs)
        (spec_dir / "index.html").write_text(page, encoding="utf-8")
        print(f"  ✓ {spec}: {n_priv} private, {len(nhs_recs)} NHS")

    print(f"\n✅ All specialist pages built in {OUT_DIR}/")
    print("Commit specialists/ and push to deploy.")
