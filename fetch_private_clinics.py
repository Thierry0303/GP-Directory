#!/usr/bin/env python3
"""
fetch_private_clinics.py  —  v5 (Valid API Schema & Robust Pipelines)
====================================================================
Pulls independently-operated, doctor-led, currently-registered
CQC locations in London using valid CQC public API parameters.
"""
...
def cqc_get(path, params, key, retries=3):
    ...
    headers = {
        ...
        "User-Agent": "londongp.directory/5.0",
    }
    for attempt in range(retries):
        try:
            ...
            if e.code == 400:
                # Provide internal warning for bad query logic parameters
                print(f"\nAPI rejected parameters on {url}")
                return None
...
# ── Clean Pagination Step ────────────────────────────────────────────────────
def paginate_london(key):
    print("Paginating CQC /locations (Clean London Extract) …")
    candidates = []
    page = 1
    per_page = 100  # Strict API max parameter allowance
    
    while True:
        # Querying only clean pagination parameters to avoid 400 validation drops
        params = {
            "page": page, 
            "perPage": per_page
        }
        ...
        if page % 50 == 0:
            print(f"  page {page}/{total_pages} — identified candidates: {len(candidates)}")
        if page >= total_pages: break
        page += 1
        time.sleep(0.05)
        
    print(f"\nFound {len(candidates)} total potential candidates in postcodes.\n")
    return candidates

# ── Detailed Parsing Engine ──────────────────────────────────────────────────
def build_records(candidates, key, nhs_ods, workers=12):
    print(f"Processing target structural analysis for {len(candidates)} records …")
    records = []
    rejected = Counter()
    done = 0
    ...
    with ThreadPoolExecutor(max_workers=workers) as pool:
        ...
        for fut in as_completed(futures):
            ...
            # Filtering out public infrastructures down here in the details payload
            ...
            if done % 200 == 0 or done == len(futures):
                print(f"  {done}/{len(futures)} analyzed — directory size: {len(records)}")

    print(f"\nExclusion metrics summary: {dict(rejected)}")
    return records
...
def main():
    ...
    if not key:
        sys.exit("Error: CQC_KEY variable environment unset.")
    ...
    if GPS_JSON.exists():
        try:
            ...
            print(f"Warning: context gps.json was unreadable — {e}")
            
    print(f"Loaded {len(nhs_ods)} ODS exclusions from dataset.\n")
    ...
    print("\nFinal Directives breakdown:")
    for sp, n in by_spec.most_common():
        print(f"  {sp:<25} {n}")

    OUT_JSON.write_text(json.dumps(records, indent=2, ensure_ascii=False))
    print(f"\nSuccess. Wrote {OUT_JSON} ({len(records)} providers mapped, {OUT_JSON.stat().st_size // 1024} KB)")
